<div class="flex-1 p-6 overflow-auto">
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-semibold text-gray-800">Laporan Pemasukan</h1>
        
        <div class="flex space-x-2">
            <a href="<?= base_url('pemasukan/cetak_laporan?tanggal_mulai='.$tanggal_mulai.'&tanggal_akhir='.$tanggal_akhir) ?>" target="_blank" class="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg flex items-center">
    ...
    <div class="bg-white rounded-lg shadow p-6 mb-6">
        <form action="<?= base_url('pemasukan/laporan') ?>" method="get" class="flex flex-wrap md:flex-nowrap items-end gap-4">
            <div class="w-full md:w-1/3">
                <label for="tanggal_mulai" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Mulai</label>
                <input type="date" id="tanggal_mulai" name="tanggal_mulai" value="<?= $tanggal_mulai ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
            </div>
            
            <div class="w-full md:w-1/3">
                <label for="tanggal_akhir" class="block text-sm font-medium text-gray-700 mb-1">Tanggal Akhir</label>
                <input type="date" id="tanggal_akhir" name="tanggal_akhir" value="<?= $tanggal_akhir ?>" class="w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring focus:ring-primary-500 focus:ring-opacity-50">
            </div>
    ...
    <div class="bg-white rounded-lg shadow p-4 mb-6 border-l-4 border-green-500">
        <div class="flex items-center">
            ...
            <div>
                <p class="text-sm text-gray-500 font-medium">Total Pemasukan (<?= date('d M Y', strtotime($tanggal_mulai)) ?> - <?= date('d M Y', strtotime($tanggal_akhir)) ?>)</p>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php if(empty($pemasukan)): ?>
                        <tr>
                            <td colspan="7" class="px-6 py-4 text-center text-gray-500">Tidak ada data pemasukan pada periode yang dipilih</td>
                        </tr>
                    <?php else: ?>
